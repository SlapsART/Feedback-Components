...
| 2026-07-09 | **Para que se vea claramente un elemento "viajar desde atrás de otro" (ej. el toast emergiendo detrás del dock del Asistente), la curva de easing importa tanto como la distancia**. Con una curva muy front-loaded (`[0.22,1,0.36,1]`, tipo easeOutExpo) el 90% del recorrido pasa en el primer 15-20% de la duración — a los 10ms de un viaje de 64px ya solo quedaban ~3px por recorrer, haciendo el "pase por detrás" imperceptible entre frames. Cambiar a `ease:'easeInOut'` distribuye el recorrido de forma pareja (a mitad de la duración, el elemento está a mitad de camino) — deja tiempo suficiente para que el elemento se vea claramente detrás/superpuesto con el elemento vecino antes de asentarse. **Cómo verificar**: muestrear `getComputedStyle(el).transform` (el valor `matrix(...)` incluye el desplazamiento `y` real) en 6-8 puntos del timeline vía Playwright, no solo mirar screenshots — confirma la distribución real del recorrido sin adivinar. El "pasar por detrás" en sí se logra gratis por ORDEN DEL DOM (el elemento que debe quedar "encima" — el dock/composer — debe ir DESPUÉS en el JSX que el toast; sin z-index explícito, los hermanos con `transform` pero `z-index:auto` respetan el orden del DOM) | FeedbackToast `enterDistance` prop + AsistenteComposer detrás — FeedbackAsistentePage |
